...
1. Follow the steps to set up the VM as a standalone environment above.
2. On your host machine open your network interface settings and set the DNS to the ip address of the VM. On a Mac this is done by selecting the Network icon and Network Preferences an then selecting Advanced
3. Ping the IP address of the VM and the URLs from your host machine's terminal to make sure that your DNS entry is working.
4. Visit your browser of choice and go to 'http://splash.inethihome.net' for example to view the docker container's UI.
